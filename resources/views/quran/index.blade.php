        ...
        <div x-ref="surahList" class="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-4 lg:grid-cols-3">
            @foreach ($surahs as $surah)
                <a href="{{ route('quran.show', $surah->id) }}"
                    class="surah-item group rounded-xl border border-gray-200 bg-white p-4 shadow-sm transition hover:border-emerald-500 hover:shadow-lg sm:p-5 dark:border-gray-700 dark:bg-gray-800 dark:hover:border-emerald-500">
                        ...
                        <div class="flex items-center space-x-4">
                            ...
                            <div>
                                <h4 class="text-sm font-semibold text-gray-900 sm:text-base dark:text-white">
                                    {{ $surah->nama }}</h4>
                                <p class="text-xs text-gray-600 sm:text-sm dark:text-gray-400">{{ $surah->jumlah_ayat }}
                                    Ayat</p>
                            </div>
                        </div>
                        <div
                            class="font-arabic text-xl text-gray-900 transition group-hover:text-emerald-600 sm:text-2xl dark:text-white dark:group-hover:text-emerald-400">
                            {{ $surah->nama_arab }}
                        </div>
                    </div>
                </a>
